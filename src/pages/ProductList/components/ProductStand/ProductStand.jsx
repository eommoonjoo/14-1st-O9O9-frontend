...
class ProductStand extends Component {
  ...
  getSlicedProducts = (toSliceProducts) => {
    const { page } = this.state;
    if (page * productPerPage > toSliceProducts.length)
      return toSliceProducts.slice((page - 1) * productPerPage);
    return toSliceProducts.slice(
          ...
          {pagedProducts &&
            pagedProducts.map((item) => (
              <ProductCard
                key={item.id}
                product={item}
                onWishClick={this.props.onWishClick}
              />
            ))}
        </div>
