...
class ListContainer extends Component {
  ...
  handleScroll = (event) => {
    ...
    if (event.srcElement.scrollingElement.scrollTop > 430 && !showHiddenSubs)
      this.setState({ showHiddenSubs: true });
    if (event.srcElement.scrollingElement.scrollTop <= 430 && showHiddenSubs)
  ...
  getProducts = async () => {
    ...
    if (categoryIds.subcategoryId) {
      productsData = await axios.get(
        PRODUCTS_LIST_API + `?sub=${categoryIds.subcategoryId}`
        // MAINCATEGORY_PRODUCTS_DATA_API + `?sub=${categoryIds.subcategoryId}`
      );
    } else {
      productsData = await axios.get(
        // MAINCATEGORY_PRODUCTS_DATA_API + `?main=${categoryIds.categoryId}`
        PRODUCTS_LIST_API + `?main=${categoryIds.categoryId}`
      );
    }
    ...
    return (
      <main className='ListContainer'>
          ...
          {subcategories &&
            subcategories.map((subcategory, idx) => (
              <div key={subcategory.id} className='subcategory'>
                <Link
                  to={`/list?category=${categoryId}&subcategory=${subcategory.id}`}>
        ...
        <ProductStand
          ...
          onWishClick={this.handleWishClick}
        />
      </main>
    );
  }
